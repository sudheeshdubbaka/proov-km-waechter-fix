...
# Written in 2014. Modernized style; crash and average-wear bugs fixed.
...
def car_wear(car: dict) -> float:
    """Return this car's wear percentage, or 0.0 if no service reading exists."""
...
def fleet_summary(fleet: list[dict]) -> dict:
    """Return a dict with count, due-for-service count, and average wear across the fleet."""
    total = 0.0
    due = 0
    for car in fleet:
        total += car_wear(car)
...
def print_report(fleet: list[dict]) -> None:
    """Print the nightly health report and flush to the log file."""
